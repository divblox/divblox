if (typeof component_classes['data_model_allowed_api_operation_crud_data_series'] === "undefined") {
	class data_model_allowed_api_operation_crud_data_series extends DivbloxDomBaseComponent {
		constructor(inputs,supports_native,requires_native) {
			...
			this.total_items = 0;
			this.current_sort_column = ["IsActive",true];
		}
		reset(inputs) {
			this.current_page_array = [];
		...
		addRow(row_data_obj) {
			...
			let html = '<a href="#" id="'+this.uid+'_row_item_'+row_id+'" class="list-group-item list-group-item-action' +
				...
				'                <h5 class="mb-1">'+row_data_obj['ApiOperation']+'</h5>\n' +
				
			'            </div>\n' +
			
				'            <small>'+is_active+'</small>\n' +
			'</a>';
			getComponentElementById(this,"DataList").append(html);
		}
		// Data table functions to implement
		on_item_clicked(id) {
			setGlobalConstrainById("AllowedApiOperation",id);
			pageEventTriggered("allowed_api_operation_clicked",{id:id});
		}
	}
