...
for ($i = 0; $i < $TicketDataSize; $i++) {
    ...
    if (0 < $Random && $Random <= $PropArr[0]) {
        $TicketObj->TicketStatus = $PropArr[0];
    } else if ($PropArr[0] < $Random &&
                $Random <= $PropArr[1]) {
        $TicketObj->TicketStatus = $PropArr[1];
    } else if ($PropArr[1] < $Random &&
                $Random <= $PropArr[2]) {
        $TicketObj->TicketStatus = $PropArr[2];
    } else if ($PropArr[2] < $Random &&
                $Random <= $PropArr[3]) {
        $TicketObj->TicketStatus = $PropArr[3];
    } else if ($PropArr[3] < $Random &&
                $Random <= $PropArr[4]) {
        $TicketObj->TicketStatus = $PropArr[4];
    } else if ($PropArr[4] < $Random &&
                $Random <= $PropArr[5]) {
        $TicketObj->TicketStatus = $PropArr[5];
    }
