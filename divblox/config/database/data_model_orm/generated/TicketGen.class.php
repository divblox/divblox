 ...
 * @property integer $TicketStatus the value for intTicketStatus 
 ...
 * @property TicketStatus $TicketStatusObject the value for the TicketStatus object referenced by intTicketStatus 
...
class TicketGen extends dxBaseClass implements IteratorAggregate {
     ...
     * @var integer intTicketStatus
     */
    protected $intTicketStatus;
    ...
    /**
     * Protected member variable that contains the object pointed by the reference
     * in the database column Ticket.TicketStatus.
     *
     * NOTE: Always use the TicketStatusObject property getter to correctly retrieve this TicketStatus object.
     * (Because this class implements late binding, this variable reference MAY be null.)
     * @var TicketStatus objTicketStatusObject
     */
    protected $objTicketStatusObject;
    ...
    public function Initialize() {
        ...
        $this->intTicketStatus = Ticket::TicketStatusDefault;
    ...
    */
    public static function InstantiateDbRow($objDbRow, $strAliasPrefix = null, $objExpandAsArrayNode = null, $objPreviousItemArray = null, $strColumnAliasArray = array()) {
        ...
        $strAliasName = !empty($strColumnAliasArray[$strAlias]) ? $strColumnAliasArray[$strAlias] : $strAlias;
        $objToReturn->intTicketStatus = $objDbRow->GetColumn($strAliasName, 'Integer');
        ...
        // Check for TicketStatusObject Early Binding
        $strAlias = $strAliasPrefix . 'TicketStatus__Id';
        $strAliasName = !empty($strColumnAliasArray[$strAlias]) ? $strColumnAliasArray[$strAlias] : $strAlias;
        if (!is_null($objDbRow->GetColumn($strAliasName))) {
            $objExpansionNode = (empty($objExpansionAliasArray['TicketStatus']) ? null : $objExpansionAliasArray['TicketStatus']);
            $objToReturn->objTicketStatusObject = TicketStatus::InstantiateDbRow($objDbRow, $strAliasPrefix . 'TicketStatus__', $objExpansionNode, null, $strColumnAliasArray);
        }
    ...
    */
     ...
     * @param integer $intTicketStatus
     * @param dxQueryClause[] $objOptionalClauses additional optional dxQueryClause objects for this query
     * @return Ticket[]
    */
    public static function LoadArrayByTicketStatus($intTicketStatus, $objOptionalClauses = null) {
        // Call Ticket::QueryArray to perform the LoadArrayByTicketStatus query
        try {
            return Ticket::QueryArray(
                dxQuery::Equal(dxQueryN::Ticket()->TicketStatus, $intTicketStatus),
                $objOptionalClauses);
        } catch (dxCallerException $objExc) {
            $objExc->IncrementOffset();
            throw $objExc;
     ...
     * @return int
    */
    public static function CountByTicketStatus($intTicketStatus) {
    ...
    */
    public function Save($blnForceInsert = false, $blnForceUpdate = false) {
        ...
        if (!$ExistingObj) {
            ...
            $ChangedArray = array_merge($ChangedArray,array("TicketStatus" => $this->intTicketStatus));
            ...
            if ($ExistingObj->TicketStatus != $this->intTicketStatus) {
                $ChangedArray = array_merge($ChangedArray,array("TicketStatus" => array("Before" => $ExistingValueStr,"After" => $this->intTicketStatus)));
                //$ChangedArray = array_merge($ChangedArray,array("TicketStatus" => "From: ".$ExistingValueStr." to: ".$this->intTicketStatus));
            }
            $ExistingValueStr = "NULL";
        ...
        try {
            if ((!$this->__blnRestored) || ($blnForceInsert)) {
                ...
                INSERT INTO `Ticket` (
							...
							' . $objDatabase->SqlVariable($this->intTicketStatus) . ',
            ...
            UPDATE `Ticket` SET
							...
							`TicketStatus` = ' . $objDatabase->SqlVariable($this->intTicketStatus) . ',
    ...
    public function Reload() {
        ...
        $this->TicketStatus = $objReloaded->TicketStatus;
    ...
    public function __get($strName) {
        switch ($strName) {
            ...
            case 'TicketStatus':
                /**
                 * Gets the value for intTicketStatus 
                 * @return integer
                 */
                return $this->intTicketStatus;
            ...
            case 'TicketStatusObject':
                /**
                 * Gets the value for the TicketStatus object referenced by intTicketStatus 
                 * @return TicketStatus
                 */
                try {
                    if ((!$this->objTicketStatusObject) && (!is_null($this->intTicketStatus)))
                        $this->objTicketStatusObject = TicketStatus::Load($this->intTicketStatus);
                    return $this->objTicketStatusObject;
                } catch (dxCallerException $objExc) {
                    $objExc->IncrementOffset();
                    throw $objExc;
                }
    ...
    public function __set($strName, $mixValue) {
        switch ($strName) {
            ...
            case 'TicketStatus':
                /**
                 * Sets the value for intTicketStatus 
                 * @param integer $mixValue
                 * @return integer
                 */
                try {
                    $this->objTicketStatusObject = null;
                    return ($this->intTicketStatus = dxType::Cast($mixValue, dxType::Integer));
                } catch (dxCallerException $objExc) {
                    $objExc->IncrementOffset();
                    throw $objExc;
                }
            ...
            case 'TicketStatusObject':
                /**
                 * Sets the value for the TicketStatus object referenced by intTicketStatus 
                 * @param TicketStatus $mixValue
                 * @return TicketStatus
                 */
                if (is_null($mixValue)) {
                    $this->intTicketStatus = null;
                    $this->objTicketStatusObject = null;
                    return null;
                } else {
                    // Make sure $mixValue actually is a TicketStatus object
                    try {
                        $mixValue = dxType::Cast($mixValue, 'TicketStatus');
                    } catch (dxInvalidCastException $objExc) {
                        $objExc->IncrementOffset();
                        throw $objExc;
                    }

                    // Make sure $mixValue is a SAVED TicketStatus object
                    if (is_null($mixValue->Id))
                        throw new dxCallerException('Unable to set an unsaved TicketStatusObject for this Ticket');

                    // Update Local Member Variables
                    $this->objTicketStatusObject = $mixValue;
                    $this->intTicketStatus = $mixValue->Id;
...
///////////////////////////////
    ...
    public static function GetSoapComplexTypeXml() {
        ...
        $strToReturn .= '<element name="TicketStatusObject" type="xsd1:TicketStatus"/>';
    ...
    public static function AlterSoapComplexTypeArray(&$strComplexTypeArray) {
        if (!array_key_exists('Ticket', $strComplexTypeArray)) {
            ...
            TicketStatus::AlterSoapComplexTypeArray($strComplexTypeArray);
        }
    }
    ...
    public static function GetObjectFromSoapObject($objSoapObject) {
        ...
        if ((property_exists($objSoapObject, 'TicketStatusObject')) &&
            ($objSoapObject->TicketStatusObject))
            $objToReturn->TicketStatusObject = TicketStatus::GetObjectFromSoapObject($objSoapObject->TicketStatusObject);
    ...
    public static function GetSoapObjectFromObject($objObject, $blnBindRelatedObjects) {
        ...
        if ($objObject->objTicketStatusObject)
            $objObject->objTicketStatusObject = TicketStatus::GetSoapObjectFromObject($objObject->objTicketStatusObject, false);
        else if (!$blnBindRelatedObjects)
            $objObject->intTicketStatus = null;
        return $objObject;
    }
    ...
    public function getIterator() {
        ...
        $iArray['TicketStatus'] = $this->intTicketStatus;
...
/////////////////////////////////////
     ...
     * @property-read dxQueryNodeTicketStatus $TicketStatusObject
	...
	class dxQueryNodeTicket extends dxQueryNode {
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case 'TicketStatus':
					return new dxQueryNode('TicketStatus', 'TicketStatus', 'Integer', $this);
				case 'TicketStatusObject':
					return new dxQueryNodeTicketStatus('TicketStatus', 'TicketStatusObject', 'Integer', $this);
				case 'ObjectOwner':
